...
def show_trame(
    ...
    **kwargs,
    ...
    animate : bool, default: False
        Opt-in continuous re-rendering loop. **Expensive** — the
        viewer redraws unconditionally regardless of whether the
        scene changed, which pegs a CPU core and floods the
        websocket. Only enable when the scene is mutated from a
        background thread or external process and you cannot call
        :meth:`~trame_pyvista.ui.base_viewer.BaseViewer.update`
        from the mutating code path. See
